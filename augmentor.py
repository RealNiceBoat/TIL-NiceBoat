...
class augmentor():
    def __init__(self,funcs,probs):
        self.funcs = []
        for f,p in zip(funcs,probs):
            self.funcs.append((f,p))
    
    def __call__(self,img,labels):
        for f,p in self.funcs:
            if p > np.random.uniform():
                img,labels = f(img,labels)
        return np.array(img),labels

'''
Usage:
aug_default = augmentor(
    [aug_colorbalance,aug_contrast,aug_brightness,aug_sharpness,aug_horizontal_flip,aug_crop,aug_translate],
